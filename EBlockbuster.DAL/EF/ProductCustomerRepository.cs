...
/*        public Response<ProductCustomer> Get(int productId, int customerId)
        {
            ...
            try
            {
                using (var db = new AppDbContext(Dbco))
                {
                    var productCustomer = db.ProductCustomers.Find(productId, customerId);
                    if (productCustomer != null)
                    {
                        response.Data = productCustomer;
                        response.Success = true;
                        response.Message = "Product and Customer found";
                    }
                    else
                    {
                        response.Success = false;
                        response.Message = "Product and Customer not found";
                    }
                    return response;
                }
            }
